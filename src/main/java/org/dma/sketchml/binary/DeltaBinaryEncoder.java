package org.dma.sketchml.binary;
...
public class DeltaBinaryEncoder implements BinaryEncoder, Serializable {
    ...
    private int writePrev;
    private static final int DEFAULT_INIT_CAPACITY = 1000;

    public DeltaBinaryEncoder(int capacity) {
        this.size = 0;
        this.flagBits = new BitSet(capacity * 2);
        this.deltaBits = new BitSet(capacity * 12);
        this.readOffset = 0;
        this.writeOffset = 0;
        this.readPrev = 0;
        this.writePrev = 0;
    ...
    public void encode(int value) {
        int delta = value - writePrev;
        int bytesNeeded = needBytes(delta);
        setBits(flagBits, 2 * size, bytesNeeded - 1, 2);
        setBytes(deltaBits, writeOffset, delta, bytesNeeded);
        writePrev = value;
        writeOffset += bytesNeeded * 8;
        size++;
    }
